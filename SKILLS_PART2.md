...
**已选择方案**：
- [ ] 相册封面选择策略：第一张媒体 vs 用户指定 vs 最近添加（待确认）
- [ ] 相册排序方式：创建时间 vs 名称 vs 自定义排序（待确认）
...
**已选择方案**：
- [x] 标签颜色功能：**保留**（数据库字段已存在，UI 支持显示）
- [ ] 标签选择器展示方式：树形展开 vs 扁平列表 + 面包屑（待确认）
- [ ] 多标签筛选模式：AND（交集）vs OR（并集）vs 用户切换（待确认）
...
**已选择方案**：
- [x] 笔记功能范围：**简单文本**（当前实现）
- [x] 笔记入口位置：**媒体查看器内**（当前实现）
- [ ] 富文本/Markdown 支持（未来扩展）
...
**已选择方案**：
- [x] 设置持久化方案：**SQLite**（Rust 侧统一管理）
- [ ] 设置数据同步：本地 only vs 云同步（待确认）
...
**已选择方案**：
- [x] 文件浏览器：**file_picker** 8.0.7（系统文件选择器）
- [ ] 导入并发策略：串行 vs 并行（限制并发数）（待确认）
- [ ] 导入事务策略：单文件事务 vs 批量事务（待确认）
